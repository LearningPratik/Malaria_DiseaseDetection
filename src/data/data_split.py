...
def get_images_fps(imgs_root):
    ...
    img_df = pd.DataFrame(
        {"img_fp" : list(map(str, Path(imgs_root).rglob("*.png")))}
    )

    img_df["is_parasitized"] = img_df['img_fp'].map(
        lambda fp: int('parasitized' is os.path.basename(fp).lower())
    )
...
if __name__ == '__main__':
    ...
    dataset_root = "data/cell_data"
    train_root = os.path.join(dataset_root, "train")
    test_root = os.path.join(dataset_root, "test")

    splits_dest_dir = "data/data_splits"
